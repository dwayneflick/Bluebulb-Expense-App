...
import { Loader2, Check, X, FileCheck2 } from "lucide-react";
...
export default function ExpenseApproval() {
  ...
  const handleActionClick = (expense: any, action: 'approve' | 'reject' | 'pay') => {
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-sm mt-1">There are no expenses pending your approval right now.</p>
                      </div>
                    </TableCell>
                  </TableRow>
                ) : (
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-4">
          <div className="bg-slate-50 p-4 rounded-xl text-sm space-y-2 border border-slate-100">
            ...
            <div className="flex justify-between"><span className="text-slate-500">Purpose</span> <span className="font-medium text-slate-800">{expense.purpose}</span></div>
          ...
          {action === 'pay' && (
            <div className="space-y-2">
              <label className="text-sm font-semibold">Receipt URL (Optional)</label>
              <input 
                type="text" 
                value={receiptUrl} 
